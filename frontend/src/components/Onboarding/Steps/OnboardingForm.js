...
export default function OnboardingForm({
  ...
  return (
    <Dialog open={true} onClose={onClose}>
      <DialogTitle><b>Record Retrieval Request</b></DialogTitle>
      <DialogContent>
        <FormGroup>
          ...
          />
          <FormControlLabel
            control={<Checkbox checked={plaidEnabled} onChange={(event) => setPlaidEnabled(event.target.checked)} />}
          ...
          <FormControlLabel
            control={<Checkbox checked={covieEnabled} onChange={(event) => setCovieEnabled(event.target.checked)} />}
          ...
          <FormControl style={{marginBottom:'40px'}}>
            <FormLabel id="demo-radio-buttons-group-label">Sharing Preference</FormLabel>
            <RadioGroup
              aria-labelledby="demo-radio-buttons-group-label"
              defaultValue="openForm"
              name="radio-buttons-group"
            >
              <FormControlLabel value="link" control={<Radio />} label="Share via link" />
              <FormControlLabel value="openForm" control={<Radio />} label="Open form in browser" />
            </RadioGroup>
          </FormControl>
        </FormGroup>
        <DialogActions>
          <Button variant="outlined" style={{ marginLeft: '8px', marginRight: '160px', width: '21em' }} onClick={onClose}>
            Back
          </Button>
          <Button variant="contained" onClick={onSuccess} style={{width: '21em'}}>
            Request Records
          </Button>
        </DialogActions>
      </DialogContent>
